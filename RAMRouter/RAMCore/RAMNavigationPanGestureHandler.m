//
//  RAMNavigationPanGestureHandler.m
//  Pods
//
//  Created by 裘俊云 on 2019/1/17.
//

#import "RAMNavigationPanGestureHandler.h"
#import "RAMNavigationBackPanGestureProtocol.h"
#import "UIViewController+RAMUtils.h"

@interface RAMNavigationPanGestureHandler () <
UIGestureRecognizerDelegate,
RAMNavigationBackPanGestureProtocol>
...
@implementation RAMNavigationPanGestureHandler
...
/*!
 *  获取viewcontroller topmost viewcontroller，如果这个controller不支持RAMNavigationBackPanGestureProtocol，则返回nil
 *
 *  @return viewcontroller
 */
- (id<RAMNavigationBackPanGestureProtocol>)topmostConformedViewController {
    ...
    while (viewController) {
        if ([viewController conformsToProtocol:@protocol(RAMNavigationBackPanGestureProtocol)]){
            lastConformedViewController = viewController;
        }
        
        UIViewController *currentViewController = [viewController ram_currentChildViewController];
    ...
    return (id<RAMNavigationBackPanGestureProtocol>)lastConformedViewController;
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIPanGestureRecognizer *)gestureRecognizer {
    ...
    id<RAMNavigationBackPanGestureProtocol> topmostViewController = [self topmostConformedViewController];
...
- (void)panGestureHandlerReceiveEnded:(UIPanGestureRecognizer*)gestureRecognizer isCanceled:(BOOL)isCanceled {
    id<RAMNavigationBackPanGestureProtocol> topmostViewController = [self topmostConformedViewController];
    if ([topmostViewController respondsToSelector:@selector(navigationControllerBackPanGestureHandlerReceiveEnded:isCanceled:)]){
        [topmostViewController navigationControllerBackPanGestureHandlerReceiveEnded:gestureRecognizer isCanceled:isCanceled];
    }
